...
public class ChainletMatrix {
    ...
    private static void getmatrix(String coin) throws Exception {
        String dir = "D:\\" + coin + "/createddata/hourly/";
        String occDir = "D:\\" + coin + "/createddata/hourlyOccMatrices/";
        String amoDir = "D:\\" + coin + "/createddata/hourlyAmoMatrices/";
        ...
        for (int year = 2009; year <= 2017; year++) {
            ...
            for (int day = 1; day <= timePeriodMax; day++) {
                for (int hour = 0; hour <= 23; hour++) {
                    ...
                    for (Long[] row : amoM)
                        Arrays.fill(row, 0L);
                    String fileName = dir + year + "_" + day + "_" + hour + ".txt";
                    ...
                    wrinfo.append(year + "\t" + day + "\t" + (long) total + "\t" + merge / total + "\t" + split / total + "\t" + transition / total + "\r\n");
                    if (total > 0) {
                        writeMatrix(occM, occDir, "occ" + year + "_" + day + "_" + hour);
                        writeMatrix(amoM, amoDir, "amo" + year + "_" + day + "_" + hour);
                    }
                }
            }
        }
    ...
    private static <T extends Number> void writeMatrix(T[][] matrix, String dir, String file) throws Exception {
        String x = "";

        BufferedWriter wr = new BufferedWriter(new FileWriter(dir + file + ".csv"));
        StringBuffer bf = new StringBuffer();
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix.length - 1; j++) {
                bf.append(matrix[i][j] + ",");
            }
            bf.append(matrix[i][matrix.length - 1]);
            bf.append("\r\n");
        }
